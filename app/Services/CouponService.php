...
class CouponService
{
    ...
    public function validate(string $code, float $subtotal, string $customerPhone): array
    {
        ...
        if (! $coupon) throw new \InvalidArgumentException('Cupom não encontrado ou inativo.');
        if (! $coupon->isValid()) throw new \InvalidArgumentException('Este cupom está expirado ou esgotado.');
        if ($subtotal < $coupon->min_order_value) throw new \InvalidArgumentException(sprintf('Subtotal mínimo para este cupom: R$ %.2f.', $coupon->min_order_value));
        if ($this->coupons->hasUsedCoupon($coupon->id, $customerPhone)) throw new \InvalidArgumentException('Você já utilizou este cupom.');
